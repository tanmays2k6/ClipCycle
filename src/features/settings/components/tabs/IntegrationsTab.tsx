...
const IntegrationCard = ({ id, name, description, isConnected, icon: Icon, color, update }: any) => (
  <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 gap-4 hover:bg-surface-hover/30 transition-colors">
    <div className="flex items-start gap-4">
      <div className="w-12 h-12 rounded-xl bg-card flex items-center justify-center shrink-0 border border-border/50 p-2.5 shadow-sm">
        <Icon className="w-full h-full" style={{ color }} />
      </div>
      <div>
        <div className="flex items-center gap-2">
          <h4 className="text-h3">{name}</h4>
          {isConnected && (
    ...
    <button
      ...
      className={cn(
        "btn-base w-full sm:w-auto",
        isConnected
          ? "btn-outline text-text-secondary hover:text-red-500 hover:border-red-500/30"
          : "btn-primary"
      )}
    >
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-4">
        <h3 className="text-h3 px-1">Connected Apps</h3>
