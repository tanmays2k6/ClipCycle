...
const ThemeCard = ({ id, label, icon: Icon, isSelected, update }: any) => (
  <button
  ...
  >
    <div className={cn(
      ...
      id === "light" ? "bg-[#F8FAFC] border-border" :
...
export function AppearanceTab({ data, update }: { data: any, update: (data: any) => void }) {  const ACCENT_COLORS = [
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <ThemeCard id="light" label="Light" icon={Sun} isSelected={data?.theme === "light"} update={update} />
          <ThemeCard id="dark" label="Dark" icon={Moon} isSelected={data?.theme === "dark"} update={update} />
          <ThemeCard id="system" label="System" icon={Monitor} isSelected={data?.theme === "system"} update={update} />
        </div>
      </div>
